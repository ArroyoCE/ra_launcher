...
/// Class that implements hashing for Atari Lynx ROMs
class LynxHashIntegration {
  // Constants
  ...
  /// Process Lynx files in parallel using compute pool
  Future<Map<String, String>> hashLynxFilesInFolders(List<String> folders) async {
    final Map<String, String> hashes = {};
    List<String> validExtensions = ['.lnx'];
...
class ComputePool {
  ...
  Future<Map<String, String>> processFiles(List<File> files) async {
    const int actualBatchSize = LynxHashIntegration.batchSize;
  ...
  static void _isolateEntryPoint(SendPort mainSendPort) {
    ...
    receivePort.listen((message) async {
      ...
      final int batchSize = message.length > 2 ? message[2] as int : LynxHashIntegration.batchSize;
      
      final Map<String, String> batchHashes = {};
      final hasher = LynxHashIntegration();
      ...
      for (int i = 0; i < files.length; i += batchSize) {
        ...
        for (final file in batch) {
          try {
            ...
            final fileHash = await hasher.hashLynx(bytes);
