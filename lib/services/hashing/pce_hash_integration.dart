...
/// Class that implements hashing for PC Engine ROMs
class PceHashIntegration {
  // Constants
  ...
  /// Process PCE files in parallel using compute pool
  Future<Map<String, String>> hashPceFilesInFolders(List<String> folders) async {
    final Map<String, String> hashes = {};
    List<String> validExtensions = ['.pce', '.sgx'];
...
class ComputePool {
  ...
  Future<Map<String, String>> processFiles(List<File> files) async {
    const int actualBatchSize = PceHashIntegration.batchSize;
  ...
  static void _isolateEntryPoint(SendPort mainSendPort) {
    ...
    receivePort.listen((message) async {
      ...
      final int batchSize = message.length > 2 ? message[2] as int : PceHashIntegration.batchSize;
      
      final Map<String, String> batchHashes = {};
      final hasher = PceHashIntegration();
      ...
      for (int i = 0; i < files.length; i += batchSize) {
        ...
        for (final file in batch) {
          try {
            ...
            final fileHash = await hasher.hashPCE(bytes);
